      ...
      <div className="min-w-0 w-full">
        <div className="min-w-0 w-full">
          <div className="mb-6 flex flex-col gap-4 @sm/main:flex-row @sm/main:justify-between @sm/main:items-center">
            <h1 className="text-2xl font-bold text-neutral-900 dark:text-white">All Activity</h1>
          </div>
          <div className="card border-violet-500/50 p-4 @sm/main:p-6 border-violet-500/50 shadow-[0_0_15px_rgba(139,92,246,0.3)]">
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-neutral-700 dark:text-gray-300">Loading activity...</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {reputationHistory.length > 0 ? (
            <div className="space-y-3">
              {reputationHistory.map((history) => (
                <div key={history.id} className="flex items-center justify-between p-4 bg-neutral-100 dark:bg-neutral-700/50 rounded-lg border border-violet-500/30 hover:bg-neutral-600/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-violet-500/20 rounded-lg">
                      <FiThumbsUp className="w-4 h-4 text-violet-700 dark:text-violet-300" />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-violet-700 dark:text-violet-300">{getActivityDescription(history.action)}</p>
                      <p className="text-xs text-neutral-600 dark:text-gray-400 mt-0.5">
            ...
            <div className="text-center py-8">
              <p className="text-neutral-600 dark:text-gray-400 text-lg">No activity yet</p>
              <p className="text-sm text-gray-500 mt-2">Start participating in the community to see your activity here</p>
              <Link 
                to="/community" 
                className="mt-4 inline-block px-4 py-2 bg-violet-500 text-neutral-900 dark:text-white rounded-lg hover:bg-violet-600 transition-colors"
              >
                Explore Community
              </Link>
            </div>
