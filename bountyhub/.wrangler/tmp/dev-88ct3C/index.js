...
app7.post(async (c) => {
  const db = createDb(c.env.DB);
  const { title: title2, content } = await c.req.json();
  if (!title2 || !content) {
...
app13.use("*", corsMiddleware);
app13.use("*", logger());
app13.use("*", prettyJSON());
app13.get("/", (c) => {
  return c.json({
...
app13.route("/api/notifications", notifications_default);
app13.route("/api/posts", posts_default);
app13.route("/api/users", users_default);
app13.route("/api/profile", profile_default);
app13.route("/api/wallet", wallet_default);
app13.notFound((c) => {
...
// .wrangler/tmp/bundle-P7DAvD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-P7DAvD/middleware-loader.entry.ts
