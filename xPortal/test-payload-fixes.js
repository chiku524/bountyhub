...
try {
  ...
  const maxTotalSizeMatch = postCreateContent.match(/maxTotalSize = (\d+)/);
  const maxFileSizeMatch = postCreateContent.match(/maxFileSize = (\d+)/);
  const maxSizeMatch = postCreateContent.match(/maxSize = (\d+)/);
  
  if (maxTotalSizeMatch) {
    const maxTotalSizeMB = parseInt(maxTotalSizeMatch[1]) / (1024 * 1024);
    console.log(`✅ Total media size limit: ${maxTotalSizeMB}MB`);
  }
  
  if (maxFileSizeMatch) {
    const maxFileSizeMB = parseInt(maxFileSizeMatch[1]) / (1024 * 1024);
    console.log(`✅ Individual file size limit: ${maxFileSizeMB}MB`);
  }
  
  if (maxSizeMatch) {
    const maxSizeMB = parseInt(maxSizeMatch[1]) / (1024 * 1024);
    console.log(`✅ Base64 conversion size limit: ${maxSizeMB}MB`);
  }
} catch (error) {
