...
export default function Profile() {
  ...
  return (
    <div className="h-screen w-full bg-neutral-900 flex flex-row">
      <Nav />
      <div className='flex flex-row flex-wrap w-full h-full pt-32 pl-20 overflow-y-scroll' style={{'ms-overflow-style': 'none', 'scrollbar-width': 'none'}}>
        <Link to={`/${userData.username}/posts`} className='w-1/4 h-1/3 bg-slate-100 bg-opacity-50 m-10 rounded p-3 border-green-500 border shadow-custom-green flex justify-center items-center hover:cursor-pointer hover:scale-110 duration-100' >
          <div>
            <h1 className='text-2xl font-bold text-amber-900 drop-shadow-custom'>Posts</h1>
          </div>
        </Link>
        <div className='w-1/4 h-1/3 bg-slate-100 bg-opacity-50 m-10 rounded p-3 border-green-500 border shadow-custom-green flex justify-center items-center hover:cursor-pointer hover:scale-110'>
